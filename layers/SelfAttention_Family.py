...
# 这段代码定义了一个完全注意力层（FullAttention），它执行自注意力操作
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        # 获取查询的形状信息，其中 B 表示批量大小，L 表示序列长度，H 表示注意力头数，E 表示每个头的维度
        B, L, H, E = queries.shape
        # 获取值的形状信息，其中 S 表示值的序列长度，D 表示每个值的维度
        _, S, _, D = values.shape
        # 计算缩放因子，如果未提供缩放系数，则使用默认值 1/√E，其中 E 是查询的维度
        scale = self.scale or 1. / sqrt(E)
        # 计算注意力分数，通过对查询和键的乘积求和。这里使用了 torch.einsum 函数，它执行了一个张量乘积的批量矩阵乘法
        scores = torch.einsum("blhe,bshe->bhls", queries, keys)

        if self.mask_flag:
            if attn_mask is None:
                attn_mask = TriangularCausalMask(B, L, device=queries.device)
            # 将掩码应用于注意力分数，将掩码位置的分数设置为 -∞，以便在 Softmax 操作中被抑制
            scores.masked_fill_(attn_mask.mask, -np.inf)
        # 注意力分数进行缩放并进行 Softmax 操作，然后应用 Dropout
        A = self.dropout(torch.softmax(scale * scores, dim=-1))
        # 通过查询与值的加权和，计算注意力加权的值。这里再次使用了 torch.einsum 函数
        V = torch.einsum("bhls,bshd->blhd", A, values)

        if self.output_attention:
            # 如果需要输出注意力权重，则返回注意力加权的值和注意力权重
...
# 这段代码定义了一个注意力层（AttentionLayer），该层由一个内部注意力机制组成，以及用于投影查询（queries）、键（keys）和值（values）的线性层
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None):
        super(AttentionLayer, self).__init__()

        d_keys = d_keys or (d_model // n_heads)  # 键的维度
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        # 将查询投影到多头注意力空间中，并进行维度重塑以适应多头注意力计算
        queries = self.query_projection(queries).view(B, L, H, -1)
        # 将键投影到多头注意力空间中，并进行维度重塑以适应多头注意力计算
        keys = self.key_projection(keys).view(B, S, H, -1)
        # 将值投影到多头注意力空间中，并进行维度重塑以适应多头注意力计算
        values = self.value_projection(values).view(B, S, H, -1)
        # 将投影后的查询、键、值传递给内部注意力机制模块，获取输出和注意力权重
        ...
        # 将多头注意力的输出进行维度重塑，以适应后续线性层的投影
        out = out.view(B, L, -1)
        # 将多头注意力的输出传递给线性层进行投影，并返回投影后的结果及注意力权重
